<%--
  Created by IntelliJ IDEA.
  User: 36353
  Date: 2021-04-15 0015
  Time: 20:01
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" %>
<%@ page language="java" pageEncoding="UTF-8" %>
<%@ taglib uri="/struts-tags" prefix="s" %>
<!DOCTYPE html>
<html>
<head>
    <link rel="stylesheet" href="../../resources/css/common.css">
    <link rel="stylesheet" href="../../resources/css/main.css">
    <script type="text/javascript" src="../../resources/js/jquery.min.js"></script>
    <script type="text/javascript" src="../../resources/js/colResizable-1.3.min.js"></script>
    <script type="text/javascript" src="../../resources/js/common.js"></script>
    <script type="text/javascript" src="../../resources/prompt/ymPrompt.js"></script>
    <link rel="stylesheet" id='skin' type="text/css" href="../../resources/prompt/skin/qq/ymPrompt.css"/>
</head>
<body>
<div class="container">
    <div id="search_bar" class="mt10">
        <div class="box">
            <div class="box_border">
                <%--搜索模块--%>
                <div class="box_top"><b class="pl15">筛选</b></div>
                <div class="box_center pt10 pb10">
                    <form action="/grade/list" method="post">
                        <table class="form_table" border="0" cellpadding="0" cellspacing="0">
                            <tr>
                                <%--按学生姓名查询--%>
                                <td>学生姓名</td>
                                <td>
                                    <input type="text" name="studentName" class="input-text lh25" size="20" value="<s:property value="#request.studentName"/>">
                                </td>
                                <%--按学号查询--%>
                                <td>学号</td>
                                <td>
                                    <input type="text" name="studentId" class="input-text lh25" size="20" value="<s:property value="#request.studentId"/>">
                                </td>
                                <%--按课程名查询--%>
                                <td>
                                    课程名
                                </td>
                                <td>
                                <span class="fl">
                                        <div class="select_border">
                                        <div class="select_containers ">
                                            <select name="courseId" class="select" id="_cid">
                                            <option value="0">--全部课程--</option>
                                                <s:iterator value="#request.courses" id="courses">
                                                    <option value="<s:property value="#courses.id"/>"><s:property value="#courses.name"/></option>
                                                </s:iterator>
                                            </select>
                                        </div>
                                        </div>
                                    </span>
                                </td>


                                <td><input type="submit" name="button" class="btn btn82 btn_search" value="查询"></td>
                                <td><input type="button" name="button" class="btn btn82 btn_res all_" value="清除"></td>
                                <td><input type="button" name="button" class="btn btn82 btn_add add_" value="新增"></td>
                            </tr>
                        </table>
                    </form>
                </div>
            </div>
        </div>
    </div>

    <div id="table" class="mt10">
        <div class="box span10 oh">
            <table width="100%" border="0" cellpadding="0" cellspacing="0" class="list_table">
                <tr>
                    <th width="5%">序号</th>
                    <th width="12%">学号</th>
                    <th width="10%">姓名</th>
                    <th width="8%">课程号</th>
                    <th width="18%">课程名</th>
                    <th>成绩</th>
                    <th width="7%">学分</th>
                    <th>备注</th>
                    <th width="15%">操作</th>
                </tr>
                <s:iterator value="#request.grades" id="grade" status="row">
                    <tr class="tr">
                            <%--序号--%>
                        <td class="td_center"><s:property value="#row.index+1"/></td>
                            <%--学号--%>
                        <td class="td_center"><s:property value="#grade[1]"/></td>
                            <%--姓名--%>
                        <td class="td_center"><s:property value="#grade[2]"/></td>
                            <%--课程号--%>
                        <td class="td_center"><s:property value="#grade[3]"/></td>
                            <%--课程名--%>
                        <td class="td_center"><s:property value="#grade[4]"/></td>
                            <%--成绩--%>
                        <td class="td_center"><s:property value="#grade[5]"/></td>
                            <%--学分--%>
                        <td class="td_center"><s:property value="#grade[6]"/></td>
                            <%--备注--%>
                        <td class="td_left"><s:property value="#course.remark"/></td>
                            <%--操作--%>
                        <td class="td_center">
                            <input type="button" data="<s:property value="#grade[0]"/>"
                                   class="ext_btn ext_btn_success edit_" value="编辑">　
                                <%--<input type="button" data="<s:property value="#student.id"/>"--%>
                                <%--class="ext_btn ext_btn_success show_" value="详细信息">　--%>
                            <input type="button" data="<s:property value="#grade[0]"/>"
                                   class="ext_btn ext_btn_error delete_" value="删除">　

                        </td>
                    </tr>
                </s:iterator>

            </table>
        </div>
    </div>

</div>
<script type="text/javascript">

    $(function () {
        // 下拉框
        $('#_cid').val('<s:property value="#request.courseId"/>');

    });

    var delete_id;
    $(function () {
        $(".all_").click(function () {
            refresh();
        });

        $(".add_").click(function () {
            ymPrompt.win({
                title: '添加成绩记录',
                height: '330',
                width: '500',
                dragOut: false,
                iframe: true,
                message: '/course/toAdd',
                handler: reload
            });
        });

        $(".edit_").click(function () {
            var id = $(this).attr("data");
            ymPrompt.win({
                title: '编辑课程',
                height: '330',
                width: '500',
                dragOut: false,
                iframe: true,
                message: '/grade/toUpdate?gradeId=' + id,
                handler: reload
            });
        });

        $(".delete_").click(function () {
            delete_id = $(this).attr("data");
            ymPrompt.confirmInfo({message: '您确定删除此信息？', handler: handlerDel});
        })
    });

    function handlerDel(tp) {
        if (tp === 'ok') {
            $.ajax({
                type: "GET",
                url: 'course/toDelete?course.id=' + delete_id,
                dataType: "text",
                success: function (result) {
                    reload();
                }
            });
        }
    }

    // 刷新页面
    function refresh() {
        self.location.href = "/grade/list";
    }

    // 重载页面
    function reload() {
        window.location.reload();
    }

</script>
</body>
</html>